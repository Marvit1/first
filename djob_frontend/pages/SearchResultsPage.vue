<template>
  <div class="container mx-auto p-4 max-w-[1260px]">
    ...
    <div v-if="paginatedSearchResults.length > 0">
      <div v-for="result in paginatedSearchResults" :key="result.id" class="p-4 bg-white shadow rounded-lg mb-4 hover:bg-gray-300">
...
const postsPerPage = 10; // Adjust the number of items per page as needed

const totalPages = computed(() => Math.ceil(searchResults.value.length / postsPerPage));

const pageNumbers = computed(() => {
  const numbers = [];
  const maxPagesToShow = 100; // You can adjust this number
