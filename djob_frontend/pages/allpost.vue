<template>
  <div class="flex flex-col sm:flex-row justify-center items-center sm:items-start max-w-[1260px] m-auto">
      ...
      <main class="flex flex-col items-center justify-center my-6 ">
        <h1 class="text-xl flex my-2 items-center justify-center text-gray-300 hover:text-gray-900 bg-gray-700 shadow-xl shadow-gray-600/50 hover:bg-gray-400 sm:w-[480px] w-[240px] rounded-lg">Ամբողջը</h1>
  
        <div class="flex flex-col sm:flex-col items-center justify-center">
          <div v-for="post in displayedPosts" :key="post.id" class="flex flex-col w-4/5 m-4 overflow-hidden border shadow-xl shadow-gray-600/50 rounded">
            <NuxtLink :to="`/post/${post.id}`">
              <img :src="post.get_thumbnail" alt="" class="w-full object-cover h-[300px] transition-transform hover:scale-110">
              <h3 class="font-serif  text-xl font-bold text-gray-700 p-2 hover:text-yellow-700">{{ post.name }}</h3>
              <p class="font-serif text-gray-700 p-2 text-sm hover:text-yellow-700">{{ post.text.slice(0, 500) }}</p>
            </NuxtLink>
          </div>
        </div>
      </main>
